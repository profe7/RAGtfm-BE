...
def calculate_recall_at_k(chunks: list[dict], ground_truth: list[str]) -> float:
    ...
    matched_terms = [term for term in ground_truth if contains_term(context, term)]
...
def calculate_hitrate_at_k(chunks: list[dict], ground_truth: list[str]) -> float:
    ...
    return float(any(contains_term(context, term) for term in ground_truth))
...
def calculate_relevance(answer: str, ground_truth: list[str]) -> float:
    matched_terms = [term for term in ground_truth if contains_term(answer, term)]
...
def calculate_faithfulness(answer: str, chunks: list[dict]) -> float:
    ...
    meaningful_words = [word for word in answer_words if word not in stopwords and len(word) > 2]

    if not meaningful_words:
        return 0.0

    supported_words = [word for word in meaningful_words if word in context]
...
async def evaluate_dataset(dataset: list[dict], k: int = 5, user_id: str | None = None) -> dict:
    ...
    return {
        ...
        "count": len(results),
        "summary": {
            f"recall@{k}": average([result["metrics"][f"recall@{k}"] for result in results]),
            f"hitrate@{k}": average([result["metrics"][f"hitrate@{k}"] for result in results]),
            "mrr": average([result["metrics"]["mrr"] for result in results]),
            "faithfulness": average([result["metrics"]["faithfulness"] for result in results]),
            "relevance": average([result["metrics"]["relevance"] for result in results]),
        },
        "results": results,
    }
